...
function nm_create_excerpt($content, $url=false, $forcereadmore=false) {
  ...
  if ($len == 0) {
    ...
    $class = nm_get_class_option('classpostreadmorelink');
    $class = $class ? ' class="'.$class.'"' : '';
    if ($url) {
      $readmorehtml = '<span class="'.nm_get_class_option('classpostreadmore','nm_readmore').'"><a'.$class.' href="'.$url.'">'.i18n_r('news_manager/READ_MORE').'</a></span>';
      if ($forcereadmore)
